# BKG — UNIFIED PRODUCTION MASTER PROMPT V5 — RUST + WASM FULL RECODE EDITION FÜR DEN CODING AGENT
...
Du sollst eine vollständige, persistente, getestete und produktionsreife Anwendung von Grund auf implementieren.
...
Die Anwendung muss dem Nutzer nicht nur Ergebnisse zeigen.
...
Verwende stattdessen mindestens:
...
* PostgreSQL;
* Redis;
* BullMQ-äquivalente oder Rust-native Queue-/Worker-Orchestrierung;
* S3-kompatiblen Object Storage;
* serverseitige Authentifizierung mit sicherem Session- und Rollenmodell;
* Zod-äquivalente Rust-Schema- und Validierungslogik;
...
* Vitest/Playwright-äquivalente Testabdeckung, soweit im Rust/WASM-Stack technisch passend ersetzt;
...
* Cloudflare Worker als separate Pflicht-Backend-Architektur;
...
vorkommen, gelten diese Stellen als **durch den Rust+WASM-Stack überschrieben**, soweit sie dem neuen verbindlichen Architekturentscheid widersprechen.
...
Zusätzlich zum Pflichtstack werden folgende externe Komponenten integriert:

1. **AlmostNode** aus `https://github.com/macaly/almostnode`
2. **OpenRouter** mit der dynamischen Free-Models-Collection aus  
   `https://openrouter.ai/collections/free-models`
3. **Kilo Code** aus `https://github.com/kilo-org/kilocode`
...
### Gastown
Gastown ist vollständig fachlich in BKG zu überführen, insbesondere für Studio-Organisation, operative Übersichten, Pipeline-/Workstation-Sichten, Produktionssteuerung und Admin-Kontrollflächen.
...
### Release 13A — Gastown / Wasteland / Operations Recode
- vollständiger Rust/WASM-Recode;
- vollständiger BKG-Rebrand;
...
- Gastown, Wasteland, Texel Studio, FalSprite, Kilo CLI, Achsi und weitere benannte Systeme nicht mehr als separate Produktinseln sichtbar sind;
